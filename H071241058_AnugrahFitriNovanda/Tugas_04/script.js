document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI() {
        ...
        if (state.gameInProgress && (state.playerHand.length === 0 || state.botHand.length === 0)) {
            ...
            const betAmount = state.bet; // Simpan nilai taruhan untuk kejelasan

            let message = '';

            if (playerWon) {
                state.balance += betAmount;
                message = `Anda menang ronde ini! +$${betAmount}`;
            } else {
                state.balance -= betAmount;
                message = `Anda kalah ronde ini! -$${betAmount}`;
            }
